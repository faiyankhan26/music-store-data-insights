...
SUM(il.unit_price * il.quantity) AS amount_specification
FROM invoice i 
...
for countries where the maximum numbers of purchases is shared return all genres. */

WITH popular_genrs AS (
...
SELECT * FROM popular_genrs WHERE RowNo <= 1;
